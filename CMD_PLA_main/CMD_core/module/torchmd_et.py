...
from utils.bio_utils import ATOM_TYPE, BIO_ATOM_TYPE, ATOM2BIO_MAP
...
class TorchMD_VQ_ET(nn.Module):
    ...
    def __init__(
        ...
        self.expand_embed_channels = expand_embed_channels
        ...
        self.embedding = nn.Embedding(self.max_z, hidden_channels - extra_channels, dtype=dtype)
        ...
        self.neighbor_embedding = (
            NeighborEmbedding(
                hidden_channels, expand_embed_channels, num_rbf, cutoff_lower, cutoff_upper, self.max_z, dtype
            )
    ...
    def forward(
        ...
        x = self.embedding(z)
        x = torch.cat([x, t], dim=1) if self.extra_channels > 0 else x
