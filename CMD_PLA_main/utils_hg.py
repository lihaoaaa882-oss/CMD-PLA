...
def save_model_dict(model, model_dir, msg):
    """
    Save model state_dict + optional lambda_val to model_dir/msg.pt
    Works with DataParallel (will save inner module.state_dict()).
    """
    model_path = os.path.join(model_dir, msg + '.pt')

    # 如果 model 被 DataParallel 包裹，取 module
    state = model.state_dict()
    # state 是 OrderedDict，但我们想把更多内容放进去
    save_dict = {
        'model_state': state
    }

    # 如果 model 有 lambda_val（可训练超参），存下来（数值）
    try:
        # 支持 tensor / nn.Parameter / float
        lambda_val = getattr(model, 'lambda_val', None)
        if lambda_val is None and hasattr(model, 'module'):
            lambda_val = getattr(model.module, 'lambda_val', None)
        if lambda_val is not None:
            # 转为 Python float 存储
            save_dict['lambda_val'] = float(lambda_val.detach().cpu().item())
    except Exception:
        # 不影响主流程：如果不能读取 lambda，就不存
        pass
...
def load_model_dict(model, ckpt, map_location=None):
    """
    Load a checkpoint saved by save_model_dict.
    - model: model instance to load into (may be DataParallel or raw)
    - ckpt: path to checkpoint
    - map_location: same as torch.load map_location
    Returns the model (with lambda restored if available).
    """
    ...
    # two possibilities: user saved earlier using old save_model_dict (pure state_dict),
    # or our new style (dict with 'model_state' key)
    if isinstance(checkpoint, dict) and 'model_state' in checkpoint:
        state_dict = checkpoint['model_state']
    else:
        # legacy support: they saved bare state_dict with torch.save(model.state_dict())
        state_dict = checkpoint

    # Try to load into model (handle DataParallel naming automatically)
    try:
        model.load_state_dict(state_dict)
    except RuntimeError:
        # 可能 state_dict 来自 DataParallel/module 或者目标 model 是 DataParallel
        # 尝试自动修正 keys 前缀
        new_state = {}
        for k, v in state_dict.items():
            if k.startswith('module.') and not any(x.startswith('module.') for x in model.state_dict().keys()):
                new_state[k[7:]] = v
            elif (not k.startswith('module.')) and any(x.startswith('module.') for x in model.state_dict().keys()):
                new_state['module.' + k] = v
            else:
                new_state[k] = v
        model.load_state_dict(new_state)

    # restore lambda_val if present
    if isinstance(checkpoint, dict) and 'lambda_val' in checkpoint:
        try:
            lam = torch.tensor(checkpoint['lambda_val'], dtype=torch.float32)
            # 放到模型同设备上
            device = next(model.parameters()).device
            lam = lam.to(device)
            # set to model or model.module if DataParallel
            if hasattr(model, 'module'):
                if hasattr(model.module, 'lambda_val'):
                    model.module.lambda_val.data = lam
                else:
                    # 如果 model.module 没有但 model 有，赋值给 model
                    model.lambda_val.data = lam
            else:
